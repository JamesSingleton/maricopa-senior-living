...
export const SLUG_ERROR_MESSAGES = {
  ...
  INVALID_CHARACTERS: 'Only lowercase letters, numbers, and hyphens are allowed.',
 ...
 */
export function getDocumentTypeConfig(sanityDocumentType: string): SlugValidationOptions {
 ...
 */
function validatePathStructure(slug: string, options: SlugValidationOptions): string[] {
  const errors: string[] = []
  const segments = slug.split('/').filter(Boolean)

  // Segment count validation
  if (options.segmentCount !== undefined && segments.length !== options.segmentCount) {
    errors.push(`${options.documentType} URLs must have ${options.segmentCount} segments`)
  }
 ...
 */
function validateRequiredPrefix(slug: string, options: SlugValidationOptions): string[] {
  const errors: string[] = []

  if (options.requiredPrefix && options.documentType && !slug.startsWith(options.requiredPrefix)) {
    errors.push(`${options.documentType} URLs must start with "${options.requiredPrefix}"`)
  }
 ...
 */
function validateForbiddenPatterns(slug: string, options: SlugValidationOptions): string[] {
  const errors: string[] = []
 ...
 */
function validateCustomRules(slug: string, options: SlugValidationOptions): string[] {
  const errors: string[] = []
...
export function validateSlugComprehensive(
  ...
  const segmentValidations = segments.map((segment) => validateSlugSegment(segment))
