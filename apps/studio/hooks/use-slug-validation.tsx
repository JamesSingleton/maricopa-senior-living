...
function extractSanityValidationErrors(
  ...
  return validation
    .filter((v) => (v?.path.includes('current') || v?.path.includes('slug')) && v.message)
    .map((v) => v.message)
}
 ...
 */
export function useSlugValidation(options: UseSlugValidationOptions): UseSlugValidationResult {
  const { slug, documentType: providedDocumentType, includeSanityValidation = true } = options
  ...
  const sanityValidation = useValidationStatus(publishedId || '', document?._type, true)

  // Extract Sanity slug validation errors
  const sanityValidationErrors = useMemo(
    () => extractSanityValidationErrors(sanityValidation.validation, includeSanityValidation),
  ...
  const segmentValidations = useMemo(() => validateSegments(segments), [segments])
